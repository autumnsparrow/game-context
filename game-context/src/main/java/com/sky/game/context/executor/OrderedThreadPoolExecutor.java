...
public class OrderedThreadPoolExecutor extends ThreadPoolExecutor {
    ...
    static Log LOGGER = LogFactory.getLog(OrderedThreadPoolExecutor.class);//LoggerFactory.getLogger(OrderedThreadPoolExecutor.class);
    ...
    private static final IGameSession EXIT_SIGNAL = new IGameSession() {
		...
		public boolean replaceAttribute(Object key, Object oldValue,
				Object newValue) {
			// TODO Auto-generated method stub
			return false;
		}
    ...
    private final BlockingQueue<IGameSession> waitingSessions = new LinkedBlockingQueue<IGameSession>();
     ...
     */
    private SessionTasksQueue getSessionTasksQueue(IGameSession session) {
        ...
        if (queue == null) {
           ...
           // session.setAttributeIfAbsent(TASKS_QUEUE, queue);
            SessionTasksQueue oldQueue = (SessionTasksQueue) session.setAttributeIfAbsent((Object)TASKS_QUEUE, (Object)queue);
    ...
    private void addWorker() {
        synchronized (workers) {
            if (workers.size() >= super.getMaximumPoolSize()) {
            	//logger.warn(" :"+super.getMaximumPoolSize());
            	logger.warn("MaximumPoolSize   Queue.size:"+getQueue().size()+" active:"+getActiveCount()+" task.count:"+getTaskCount()+" core.pool.size:"+getCorePoolSize()+" maximum.pool.size:"+getMaximumPoolSize());
     ...
     */
    private void print(Queue<Runnable> queue,EventProcessTask<?>  event) {
        ...
        for (Runnable elem : queue) {
            ...
            sb.append(((EventProcessTask<?>) elem).getType()).append(", ");
        }
        sb.append("]\n");
    ...
    public void execute(Runnable task) {
        ...
        EventProcessTask<?> event = (EventProcessTask<?>) task;

        // Get the associated session
        IGameSession session = event.getSession();
        ...
        boolean offerEvent = true;//eventQueueHandler.accept(this, event);
...
//        }
    ...
    private void checkTaskType(Runnable task) {
        if (!(task instanceof EventProcessTask)) {
...
//        }
    ...
    private class Worker implements Runnable {
        ...
        public void run() {
            ...
            try {
                for (;;) {
                    IGameSession session = fetchSession();
        ...
        private IGameSession fetchSession() {
...
//                Boolean focus=(Boolean)sessionTasksQueue.session.getAttribute(IGameSession.FOCUS);
//                if(focus.booleanValue()){
//                	logger.info("-------------thread["+thread.getId()+"] ,session="+sessionTasksQueue.session.getId());
//                }
                runTask(task);
            }
        }
    ...
    private static final Log logger=LogFactory.getLog(OrderedThreadPoolExecutor.class);
